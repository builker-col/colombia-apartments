...
from colombia_apartments.items import ApartmentsItem

# load cities
import urllib.parse
import yaml

with open('ciudades.yml', 'r') as file:
    ciudades = yaml.load(file, Loader=yaml.FullLoader)

ciudades = ciudades['colombia']
...
class MetrocuadradoSpider(scrapy.Spider):
        ...
        for ciudad in ciudades:
            for type in ['venta', 'arriendo']:
                for offset in range(0, 1000, 50):
                    url = f'{self.base_url}?realEstateTypeList=apartamento&realEstateBusinessList={type}&city={str(ciudad)}&from={offset}&size=50'
    ...
    def parse(self, response,):
        ...
        # if len(result) == 0:
        #     raise scrapy.exceptions.CloseSpider('No more apartments found')
    ...
    def details_parse(self, response):
        ...
        for item in script_data:
            ...
            #ciudad
            loader.add_value('ciudad', script_data['city']['nombre'])
            #tipo_propiedad
            ...
            # zona
            loader.add_value('sector', script_data['neighborhood'])
            # zona comun
            loader.add_value('sector_comun', script_data['commonNeighborhood'])
            #estrato
            ...
            loader.add_value('longitud', self.try_get(script_data, ['coordinates', 'lon']))
            #latitud
            # loader.add_value('latitud', script_data['coordinates']['lat'])
            loader.add_value('latitud', self.try_get(script_data, ['coordinates', 'lat']))
            #featured_interior
